...
import { Building2, Layers, Search, Calendar, Users, Eye, Edit2, PlaySquare } from "lucide-react";
import { getStoredBatches } from "../../../../utils/batchState";
import { Link } from "react-router-dom";

const mockColleges = [
  { id: "col_1", name: "Urvah Engineering College", mappedBatches: 1, totalStudents: 45 },
  { id: "col_2", name: "Tech Nexus Institute", mappedBatches: 1, totalStudents: 60 }
];
...
export const BatchesView = () => {
  ...
  const filteredBatches = batches.filter(
    (b) =>
      b.trainerId === "trn_1" || b.trainerId === "trn_01" || b.trainerId === "trainer_1" // Mock current trainer ID filter
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
         <div className="lg:col-span-2 flex flex-col gap-6">
            <div className="flex justify-between items-center bg-card border border-border p-4">
               <div>
                  <h3 className="text-sm font-bold text-foreground mb-1">Your Assigned Batches</h3>
                  <p className="text-[10px] font-mono text-muted-foreground">Manage classes, start sessions, and track progress.</p>
               </div>
                  ...
                  <div key={batch.id} className="border border-border bg-card p-5 flex flex-col group hover:bg-white/[0.02] hover:border-border/80 transition-colors">
                     ...
                     <div className="mt-auto flex justify-between items-end border-t border-border pt-4">
                        ...
                        <button className="px-4 py-2 bg-primary/10 text-primary hover:bg-primary/20 transition-colors text-[10px] font-bold uppercase tracking-widest flex items-center gap-2">
                           <PlaySquare className="w-3 h-3" /> Start Class
                        </button>
                     </div>
                  </div>
               ))}
