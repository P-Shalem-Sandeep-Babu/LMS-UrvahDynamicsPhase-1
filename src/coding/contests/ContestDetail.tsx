    ...
    <div className="flex flex-col gap-6 pb-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {contests.map((contest, i) => (
          <div key={i} className="border border-white/10 bg-[#080808] p-6 relative overflow-hidden group hover:border-white/20 transition-all flex flex-col">
             {contest.startsIn === "Live Now" && (
                <div className="absolute top-0 left-0 w-full h-1 bg-red-500 animate-pulse" />
             )}
             <div className="flex justify-between items-start mb-4">
                <div className="p-2 border border-white/10 bg-white/5">
                   {contest.startsIn === "Live Now" ? <Zap className="w-5 h-5 text-red-500" /> : <Trophy className="w-5 h-5 text-white/50" />}
                </div>
                <span className={`px-2 py-0.5 text-[8px] uppercase tracking-widest font-bold border ${
                   contest.type === 'Official' ? 'border-primary/50 text-primary bg-primary/10' : 'border-white/20 text-white/50 bg-white/5'
                }`}>
                   {contest.type}
                </span>
             </div>
             
             <h3 className="text-lg font-bold uppercase tracking-tight text-white mb-2">{contest.title}</h3>
             ...
             <div className="flex-1 space-y-2 mt-4 mb-6">
                ...
                <div className="flex justify-between items-center text-xs font-mono text-white/60">
                   ...
                   <span className="text-white">{contest.participants}</span>
                </div>
                <div className="flex justify-between items-center text-xs font-mono text-white/60">
             ...
             <div className="mt-auto">
                {contest.startsIn === "Live Now" ? (
                   <div className="flex gap-2">
                     <Link to="/coding/workspace" className="flex-1 text-center py-2 bg-red-500 text-white text-[10px] font-black uppercase tracking-widest hover:bg-red-600 transition-colors">
                        Enter Arena
                     </Link>
                     <Link to="/coding/contests/leaderboard" className="px-4 flex items-center justify-center py-2 border border-white/20 hover:bg-white/5 transition-colors">
                        <Trophy className="w-4 h-4 text-white/70" />
                     </Link>
                   </div>
                ) : (
                   <button className="w-full py-2 bg-white/5 text-white text-[10px] font-black uppercase tracking-widest hover:bg-white/10 transition-colors border border-white/10">
